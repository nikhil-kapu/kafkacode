...
- `--no-ai` - Disable AI-powered analysis (run pattern scan only)
...
Pattern-based scanning needs no configuration. AI-powered analysis is opt-in and
configured via environment variables (bring-your-own-key):
...
```bash
# Enable AI analysis with your own key (free tier: https://console.groq.com/keys)
export KAFKACODE_API_KEY=your_key_here
kafkacode scan ./src

# Force pattern-only, even if a key is set
